...
- **Added Direct Paystack Checkout** — new `POST /v1/orders/direct-checkout` endpoint lets users pay for orders immediately via Paystack (card, bank, USSD, etc.) without needing wallet balance. Backend initialises the Paystack transaction securely and returns `accessCode` + `authorizationUrl` for the frontend to complete payment via Popup or redirect. Also available via `POST /v1/orders/create` with `paymentMethod: "paystack_direct"`. Webhook auto-confirms orders. See `docs/direct-checkout.md`.
...
### Product System Overhaul — Inventory, Dynamic Categories & Full Admin CRUD

- **Inventory tracking** — Added `quantity_in_stock` field (integer, default 0) to all 5 product models: `Product`, `Bundle`, `MealPack`, `ReadyEatItem`, `SnackItem`. Exposed as `quantityInStock` in all API responses.
- **Dynamic categories (DB-backed)** — Created `ProductCategory` and `SnackCategory` models so the admin can add, edit, rename, reorder, or deactivate categories from the admin panel. No code changes needed to add new categories. Category dropdown in the Product/SnackItem admin forms is populated dynamically from these tables.
- **Admin improvements** — All product admins now show `quantity_in_stock` in list view and support inline editing. `BundleItemInline` shows 3 extra rows by default with autocomplete product picker. Product and Snack category admins support inline editing of name, emoji, display order, and active status.
- **API tags fix** — Bundle and ReadyEatItem API endpoints (`/v1/products/bundles`, `/v1/products/readyeat`) now read tags from the normalized `BundleTag`/`ReadyEatItemTag` tables instead of the legacy JSON field. Ensures tags added via admin inlines actually appear in the API.
- **Seed catalog management command** — Created `python manage.py seed_catalog` (and `--clear` flag) to bulk-seed all product types from canonical seed data. Temporarily disables WebSocket/cache signals during seeding for performance.
- **Signal performance fix** — Module-level `SIGNALS_DISABLED` flag in `products/signals.py` allows bulk operations to skip Redis broadcasts and cache invalidation, preventing timeouts during mass inserts.
- **Migrations applied** — `0003` (add `quantity_in_stock` to all models), `0004` (create `ProductCategory`, `SnackCategory` models, remove hardcoded choices).
- **Created `docs/product-system.md`** — full documentation of the product system architecture, admin CRUD guide, API endpoints, and management commands.
- **Phone number uniqueness enforcement on auth** — Added `intent` parameter (`"signup"` or `"login"`) to `otp-request` and `otp-verify` endpoints. Signup is rejected with `409 PHONE_ALREADY_REGISTERED` if the phone number belongs to an existing user. Login is rejected with `404 ACCOUNT_NOT_FOUND` if no account exists. Checks are enforced at both the OTP request stage (before sending SMS) and the OTP verify stage (before issuing tokens). Backwards-compatible: if `intent` is omitted, the original combined login/signup flow is preserved.

### Claude Deployment Guide

- **Created `claude-deployment.md`** — comprehensive reference document for Claude AI covering: project overview, full project structure, step-by-step deployment commands, Vercel configuration, all environment variables, database management, complete API route map, authentication system, payment system (3 order modes + DVA), product system (5 types + categories + inventory), high-throughput architecture, key technical patterns, management commands, dependencies table, workflow rules, and troubleshooting guide.

### Protected Admin & Password Change

- **Added `is_protected` field to User model** — boolean flag (default `False`). Protected users cannot be deleted from the Django admin panel (neither single delete nor bulk "delete selected" action). Main admin account is created with `is_protected=True`.
- **Admin delete protection** — `has_delete_permission()`, `delete_queryset()`, and `delete_model()` overrides in `UserAdmin` block deletion of protected users with warning messages. Even if you "Select all → Delete selected", the protected admin survives.
- **Added `PUT /v1/user/change-password` endpoint** — authenticated users can change their password by providing `currentPassword` and `newPassword` in the request body. Validates current password, enforces minimum 6 characters on new password. Migration `0003_user_is_protected` applied to Neon.
- **Switched to live Paystack keys** — redeployed with live Paystack environment variables set in Vercel dashboard.

### Production Account Number Fix

- **Removed fake 10-digit account number generation** — `generate_account_number()` function and auto-generation logic removed from `UserManager.create_user()`. New users get `account_number=None` until Paystack DVA provisions a real bank account.
- **API responses now show real DVA details** — `/v1/user/me`, `/v1/user/profile`, and `/v1/auth/otp-verify` responses now return `accountNumber`, `bankName`, `accountName` from the Paystack DVA fields (falling back to legacy fields if DVA not yet assigned). Added `dvaAssigned` boolean to all user responses.
- **Django 6.0 admin login fix** — replaced Django's `update_last_login` signal handler with a safe `queryset.update()` version in `users/apps.py` to fix UUID primary key crash on admin login.
- **Admin protected_badge fix** — switched from `format_html()` to `mark_safe()` for Django 6.0 compatibility (no placeholder args).
- **Paystack integration tests passed (8/8)** — live deployment confirmed with `pk_live_*` public key, server-init topup working, DVA endpoint responding, all verify flows handled correctly.
